...
using(var scope = app.Services.CreateScope())
{
    ...
    if(user == null)
    {
        ...
        userManager.AddClaimAsync(user, new System.Security.Claims.Claim(SharedValues.IdentityServer.Claim_Grandma, "big.cookie"))
            .GetAwaiter().GetResult();


        // 아래 두개의 클레임을 access_token에 포함시키기 위해서는 
        // ApiResouce에 UserClaim을 추가해야 한다.
        userManager.AddClaimAsync(user, new System.Security.Claims.Claim(SharedValues.IdentityServer.Claim_ApiOne_UserId, "Bruce"))
            .GetAwaiter().GetResult();
        userManager.AddClaimAsync(user, new System.Security.Claims.Claim(SharedValues.IdentityServer.Claim_ApiOne_UserGrade, "Admin"))
            .GetAwaiter().GetResult();
    }
