...
namespace OAuth.Client.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            var serverRequest = () => { return GetAsync($"{Constants.ServerAddress}/secret/index"); };
            var serverResponse = await SendMessageWrapper(serverRequest);

            await RefreshToken();

            var apiReqeust = () => { return GetAsync($"{Constants.ApiAddress}/secret/index"); };
        ...
        public async Task RefreshToken()
        {
            ...
            var request = new HttpRequestMessage(HttpMethod.Post, Constants.ServerAddress+"/oauth/token");
